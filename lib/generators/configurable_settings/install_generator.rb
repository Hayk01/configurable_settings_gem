  ...
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      def self.next_migration_number(dirname)
        @prev_migration_time ||= Time.now.utc
        @prev_migration_time += 1
        @prev_migration_time.strftime('%Y%m%d%H%M%S')
      end

      # Prepare instance vars
      def init_names
        @model_name = model_name.to_s.strip.camelize
      ...
      def validate_model_exists
        return if File.exist?(File.join(destination_root, 'app/models', "#{@base}.rb"))
